    ...
    try {
        ...
        for (let pageIdx = 0; pageIdx < 10; pageIdx++) {
            const raw = await page.evaluate(async (bookId, pgIdx, token) => {
                ...
                const resp = await fetch(url, { credentials: 'include', headers: { 'Accept': 'application/json' } });
                const data = await resp.json();
                return { code: data.code, message: data.message, list: data.data?.chapter_list || [] };
...
async function doGetPlatformInfo(cookieStr, input) {
    ...
    try {
        ...
        return { success: true, action: 'get_platform_info', workId: wkId, drafts, publishedChapters: chapterData.chapters || [], lastPublished: chapterData.lastPublished || null, volumes: volumeData };
    } finally { await browser.close().catch(() => {}); _globalBrowser = null; log('info', 'browser closed'); }
}
...
async function doPublishDraft(cookieStr, input) {
    ...
    if (!draftTitle) { fail('missing required field: draftTitle'); return; }
    if (!novelName) { fail('missing required field: novelName'); return; }

    log('info', 'publish_draft starting', { draftTitle, novelName, volumeName, itemId });
    ...
    try {
        ...
        for (let submitAttempt = 0; submitAttempt < 12; submitAttempt++) {
            ...
            try { const mh = await page.evaluate(() => { const dialogTexts = ['发布提示','是否确定提交','确认发布','确定发布']; const allSpans = document.querySelectorAll('span, div, p'); for (const el of allSpans) { const t = el.textContent.trim(); for (const dt of dialogTexts) { if (t.includes(dt) && el.offsetParent !== null) { const container = el.closest('div[class*="modal"], div[class*="dialog"], div[class*="drawer"], div[class*="overlay"]') || document.body; const btns = container.querySelectorAll('button'); for (const btn of btns) { if (btn.offsetParent === null || btn.disabled) continue; const bt = btn.textContent.trim(); if (['确认发布','确认提交','确认','确定','确定提交','提交','发布','我知道了','知道了'].some(s => bt.includes(s))) { btn.click(); return 'clicked:' + bt; } } } } } return null; }); if (mh) { log('info', 'publish confirm modal handled', { action: mh }); await sleep(4000); continue; } } catch (e) {}
    ...
    try {
        return await page.evaluate(() => {
            ...
            if (txt.includes('已达上限') || txt.includes('今日发布次数已达') || txt.includes('发布次数已用完') ||
                txt.includes('已达每日上限') || txt.includes('每日发布数量') || txt.includes('超过上限') ||
                txt.includes('今日发布') && txt.includes('上限')) {
                return true;
            }
            return false;
        });
