...
function setLogPrefix(novelName) {
    _logPrefix = 'fanqie_' + (novelName || 'unknown').replace(/[^a-zA-Z0-9\u4e00-\u9fff_-]/g, '_').substring(0, 60);
}
...
function log(level, msg, data) {
    ...
    try {
        fs.appendFileSync(LOG_DIR + '/fanqie_publish.log', line);
...
function parseChapterNumber(obj) {
    if (obj.chapter_number != null && obj.chapter_number > 0) return obj.chapter_number;
    const title = obj.title || '';
    const m = title.match(/第\s*([\d一二三四五六七八九十百千]+)\s*章/);
    if (!m) return 0;
    const raw = m[1];
...
function chineseToNumber(s) {
    ...
    let result = 0, unit = 1, tmp = 0;
    for (let i = s.length - 1; i >= 0; i--) {
        const c = s[i];
        if (c === '十') { tmp = tmp || 1; result += tmp * 10 * unit; tmp = 0; }
        else if (c === '百') { tmp = tmp || 1; result += tmp * 100 * unit; tmp = 0; }
        else if (c === '千') { tmp = tmp || 1; result += tmp * 1000 * unit; tmp = 0; }
        else if (map[c] != null) { tmp = map[c]; }
    }
    result += tmp * unit;
...
async function doSaveDraft(cookieStr, input) {
    const { title, content, novelName, chapterNumber, workId } = input;
    if (!content) { fail('missing required field: content'); return; }
    if (!novelName) { fail('missing required field: novelName'); return; }
    if (content.length < CONFIG.MIN_CONTENT_LENGTH) { throw new Error('content too short'); }
    log('info', 'save_draft starting', { title, novelName, chapterNumber, contentLen: content.length });
    ...
    try {
        ...
        // 用入参 chapterNumber + title 直接构造，不依赖 content 正则解析（格式多样，正则不可靠）
        let cleanTitle = (title || '').replace(/^第\s*[\d一二三四五六七八九十百千]+\s*章\s*/, '').trim();
        let fullTitle = '第' + effectiveChapterNumber + '章 ' + (cleanTitle || '未命名');
        let cleanContent = content
            .replace(/^#\s*第\s*[\d一二三四五六七八九十百千]+\s*章[\s\S]{0,50}?\n+/, '')
            .replace(/^#\s+.+?\n+/, '');

        await navigateToChapterEditor(page); await sleep(2000); await dismissPopups(page); await sleep(1000);
        log('info', 'filling title/content', { fullTitle, contentLen: cleanContent.length });

        await fillChapterNumber(page, effectiveChapterNumber);
        await fillTitle(page, cleanTitle);
        await fillContent(page, cleanContent);
        await sleep(1000);
        await takeScreenshot(page, 'fanqie_save_draft_before');
        const st = await dumpPageState(page, 'before save draft click');
        log('info', 'page state before save draft', st);
        await clickButton(page, ['存草稿', '保存草稿']);
        await sleep(3000);
        log('info', 'save draft success', { title: fullTitle });
        const itemId = extractItemIdFromURL(page.url());
        const result = { success: true, action: 'save_draft' };
        if (itemId) {
            result.draftItemId = itemId;
...
async function fetchDraftListViaAPI(page, workId) {
    try {
        ...
        const raw = await page.evaluate(async (bookId, token) => {
            let url = `https://fanqienovel.com/api/author/chapter/draft_list/v1?book_id=${bookId}&page_index=0&page_count=50`;
            if (token) url += '&msToken=' + encodeURIComponent(token);
            const resp = await fetch(url, { credentials: 'include', headers: { 'Accept': 'application/json' } });
            const data = await resp.json();
            return { code: data.code, message: data.message, list: data.data?.draft_list || [] };
        }, workId, msToken);
        log('info', 'draft_list API response', { workId, code: raw.code, message: raw.message, draftCount: raw.list.length });
        if (raw.code === 0 && raw.list.length > 0) {
            const drafts = raw.list.map(d => ({ title: d.title || '', chapterNumber: parseChapterNumber(d), itemId: d.item_id || '' }));
            log('info', 'drafts via API', { count: drafts.length });
            return drafts;
        }
        log('info', 'drafts via API', { count: 0 });
        return [];
    } catch (e) { log('warn', 'fetchDraftListViaAPI error', { error: e.message }); return []; }
}

async function fetchChapterListViaAPI(page, workId) {
    try {
        const msToken = await getMsToken(page);
        const chapters = []; let lastPublished = null;
        for (let pageIdx = 0; pageIdx < 10; pageIdx++) {
            const raw = await page.evaluate(async (bookId, pgIdx, token) => {
                let url = `https://fanqienovel.com/api/author/chapter/chapter_list/v1?book_id=${bookId}&page_index=${pgIdx}&page_count=50&status=0`;
                if (token) url += '&msToken=' + encodeURIComponent(token);
                const resp = await fetch(url, { credentials: 'include', headers: { 'Accept': 'application/json' } });
                const data = await resp.json();
                return { code: data.code, message: data.message, list: data.data?.item_list || [] };
...
async function fetchVolumeListViaAPI(page, workId) {
    try {
        const msToken = await getMsToken(page);
        const raw = await page.evaluate(async (bookId, token) => {
            let url = `https://fanqienovel.com/api/author/volume/volume_list/v1?book_id=${bookId}`;
            if (token) url += '&msToken=' + encodeURIComponent(token);
            const resp = await fetch(url, { credentials: 'include', headers: { 'Accept': 'application/json' } });
            const data = await resp.json();
            return { code: data.code, message: data.message, list: data.data?.volume_list || [] };
        }, workId, msToken);
        if (raw.code === 0 && raw.list.length > 0) return raw.list.map(v => ({ volumeName: v.volume_name || v.title || '', volumeId: v.volume_id || '' }));
        return [];
    } catch (e) { log('warn', 'fetchVolumeListViaAPI error', { error: e.message }); return []; }
}


async function doGetPlatformInfo(cookieStr, input) {
    const { novelName, workId } = input;
    if (!novelName) { fail('missing required field: novelName'); return; }
    ...
    try {
        ...
        if (wkId) {
            ...
            if (typeof fetchVolumeListViaAPI === 'function') { volumeData = await fetchVolumeListViaAPI(page, wkId); log('info', 'volumes via API', { count: volumeData.length }); }
            if (typeof fetchDraftListViaAPI === 'function') { drafts = await fetchDraftListViaAPI(page, wkId); log('info', 'drafts via API', { count: drafts.length }); }
        }
        if (newlyCreated) {
        ...
        if (volumeData.length === 0) {
            if (typeof extractVolumeList === 'function') { volumeData = await extractVolumeList(page); log('info', 'volumes via DOM (fallback)', { count: volumeData.length }); }
        }
        if (drafts.length === 0) {
            await clickButton(page, ['草稿箱', '草稿', '草稿管理', '我的草稿']); await sleep(3000); await dismissPopups(page); await sleep(1000);
...
async function doSetBookInfo(cookieStr, input) {
    const { name, description, category, coverBase64, workId, roles } = input;
    if (!name) { fail('missing required field: name'); return; }
    if (!coverBase64) { fail('missing required field: coverBase64'); return; }
    log('info', 'set_book_info starting', { name, category, workId });
    const browser = await launchBrowser(); let page;
    try {
        page = await browser.newPage(); await page.setViewport(CONFIG.VIEWPORT);
        await loginAndNavigate(page, cookieStr);
        await sleep(3000); await dismissPopups(page); await sleep(1000);
        const picURI = await page.evaluate(async (params) => {
            const bstr = atob(params.coverBase64);
            const bytes = new Uint8Array(bstr.length);
            for (let i = 0; i < bstr.length; i++) bytes[i] = bstr.charCodeAt(i);
            const blob = new Blob([bytes], { type: 'image/png' });
            const fd = new FormData();
            fd.append('upfile', blob, 'cover.png');

            const uResp = await fetch('https://fanqienovel.com/api/author/data/upload_pic_v1/v0', {
                method: 'POST', body: fd, credentials: 'include',
                headers: { 'Accept': 'application/json' }
            });
            const uData = await uResp.json();
            if (uData.code !== 0) throw new Error('upload_pic failed: code=' + uData.code + ' msg=' + (uData.message || '') + ' resp=' + JSON.stringify(uData));
            if (!uData.data || !uData.data.pic_uri) throw new Error('upload_pic missing pic_uri, resp=' + JSON.stringify(uData));

            const mBody = {
                aid: 2503,
                app_name: 'muye_novel',
                book_id: params.workId,
                book_name: params.name,
                thumb_uri: uData.data.pic_uri,
                abstract: params.description || '',
                gift_word: '',
                roles: params.roles ? JSON.stringify(params.roles.split(',').map(s => s.trim()).filter(Boolean)) : '',
                gender: 2,
                is_self_pic: 1,
            };
            const mFormBody = new URLSearchParams();
            for (const [k, v] of Object.entries(mBody)) { mFormBody.append(k, String(v)); }
            const mResp = await fetch('https://fanqienovel.com/api/author/book/modify_book/v0', {
                method: 'POST', credentials: 'include',
                headers: { 'Accept': 'application/json' },
                body: mFormBody
            });
            const mData = await mResp.json();
            if (mData.code !== 0) throw new Error('modify_book failed: code=' + mData.code + ' msg=' + (mData.message || '') + ' contentType=form-urlencoded req=' + mFormBody.toString() + ' resp=' + JSON.stringify(mData));
            return uData.data.pic_uri;
...
async function doPublishDraft(cookieStr, input) {
    const { draftTitle, novelName, volumeName, workId } = input;
    const itemId = input.itemId || input.draftItemId || '';
    if (!draftTitle) {
        log('warn', 'draftTitle is empty');
    }
    if (!draftTitle && !itemId) {
        fail('missing required field: both draftTitle and itemId are empty');
        return;
    }
    if (!novelName) { fail('missing required field: novelName'); return; }

    log('info', 'publish_draft starting', { draftTitle, novelName, volumeName, itemId, workId });
    ...
    try {
        ...
        if (itemId && workId) {
            log('info', 'navigating to draft editor via itemId', { itemId });
            await loginAndNavigate(page, cookieStr);
            const editUrl = CONFIG.BASE_URL + '/main/writer/' + workId + '/publish/' + itemId + '/?enter_from=modifydraft';
            await page.goto(editUrl, { waitUntil: 'domcontentloaded', timeout: CONFIG.NAVIGATION_TIMEOUT });
            await sleep(3000); await dismissPopups(page); await sleep(1000);
            await takeScreenshot(page, 'fanqie_draft_editor_loaded');
            const onEditor = await isOnEditorPage(page);
            if (!onEditor) {
                log('warn', 'draft editor not loaded via itemId, falling back to title match');
                await page.goto(CONFIG.WRITER_URL, { waitUntil: 'domcontentloaded', timeout: CONFIG.NAVIGATION_TIMEOUT });
                await sleep(3000);
                if (input.workId) { await enterNovelByWorkId(page, input.workId, novelName); }
                await sleep(2000); await dismissPopups(page); await sleep(1000);
                await clickButton(page, ['草稿箱', '草稿', '草稿管理', '我的草稿']);
                await sleep(3000); await dismissPopups(page); await sleep(1000);
                await takeScreenshot(page, 'fanqie_draft_list');
                const found = await clickDraftByTitle(page, draftTitle);
                if (!found) { await takeScreenshot(page, 'fanqie_draft_not_found'); throw new Error('在草稿箱中找不到标题为 "' + draftTitle + '" 的草稿'); }
            }
        } else {
            await loginAndNavigate(page, cookieStr);
        ...
        await sleep(6000);
        await dismissPopups(page); await sleep(1000);
        await takeScreenshot(page, 'fanqie_draft_editor_loaded');
        const onEditor2 = await isOnEditorPage(page);
        if (!onEditor2) { log('warn', 'draft editor not loaded, retrying wait'); await sleep(15000); await dismissPopups(page); }

        const capturedItemId = await page.evaluate(() => {
            const m = window.location.href.match(/\/publish\/(\d+)/);
            return m ? m[1] : '';
        });

        log('info', 'page state before clicking 下一步', { label: 'before draft publish click', ...(await dumpPageState(page, 'draft_editor')) });
        log('info', 'clicking 下一步 from draft editor');
        await clickButton(page, ['下一步']); await sleep(2000);

        const afterNextModal = await handlePublishConfirmModal(page);
        if (afterNextModal) { log('info', 'publish confirm modal handled after next step'); await sleep(2000); }

        await handleContentDetectionDialog(page); await sleep(1000);
        const typoHandled = await handleTypoCheckAndNext(page);
        if (!typoHandled) { await sleep(1000); await clickButton(page, ['下一步']); await sleep(2000); }

        log('info', 'handling publish settings page');
        await handlePublishSettings(page, volumeName); await sleep(2000);
        ...
        try {
            const handled = await page.evaluate(() => {
                ...
                for (const modal of modalEls) { if (modal.offsetParent === null) continue; const btns = modal.querySelectorAll('button'); const cts = ['确定','确认','提交','发布','我知道了','知道了','好的','是','确认发布','确定提交','立即发布','继续','下一步']; for (const btn of btns) { if (btn.offsetParent === null || btn.disabled) continue; const t = (btn.textContent || '').trim(); if (cts.some(ct => t === ct || t.includes(ct))) { btn.click(); return 'clicked:' + t; } } const nonCancelBtns = Array.from(btns).filter(b => { if (b.offsetParent === null || b.disabled) return false; const t = (b.textContent || '').trim(); return !['取消','关闭','×','x','X'].includes(t); }); if (nonCancelBtns.length > 0) { nonCancelBtns[nonCancelBtns.length - 1].click(); return 'fallback_clicked:' + nonCancelBtns[nonCancelBtns.length - 1].textContent.trim(); } }
                return null;
            });
            if (handled) { log('info', 'generic popup handled', { action: handled }); await sleep(2000); }
        } catch (e) {}

        log('info', 'entering simplified publish submit loop');
        let publishSuccessConfirmed = false;
        for (let submitAttempt = 0; submitAttempt < 12; submitAttempt++) {
            log('info', 'submit loop iteration', { attempt: submitAttempt });
            try { const state = await dumpPageState(page, 'submit_loop'); log('info', 'page state', { url: state.url, bodyPreview: state.bodyPreview, visibleBtns: state.visibleBtns }); } catch (e) {}
            try { const sc = await page.evaluate(() => { const txt = document.body.innerText; if (txt.includes('发布成功') || txt.includes('成功发布') || txt.includes('已发布') || txt.includes('审核中')) return 'text'; const url = window.location.href; if (url.includes('/chapter-manage/') || url.includes('/chapter/') || url.includes('chapter_id=') || url.includes('chapter-manage')) return 'url_chapter'; if (!url.includes('/publish/')) return 'url_left_publish'; return null; }); if (sc) { log('info', 'publish success confirmed', { indicator: sc }); publishSuccessConfirmed = true; break; } } catch (e) {}
            try { const dl = await checkDailyLimit(page); if (dl) { await takeScreenshot(page, 'fanqie_daily_limit'); fail(dl, 'DAILY_LIMIT'); return; } } catch (e) {}
            try { const mhResult = await page.evaluate(() => { const dialogTexts = ['发布提示','是否确定提交','确认发布','确定发布']; const allSpans = document.querySelectorAll('span, div, p'); for (const el of allSpans) { const t = el.textContent.trim(); for (const dt of dialogTexts) { if (t.includes(dt) && el.offsetParent !== null) { const ancestorChain = []; let p = el; while (p && p !== document.body) { ancestorChain.push(p.tagName + (p.className && typeof p.className === 'string' ? '.' + p.className.split(' ').filter(c => c).join('.') : '') + (p.id ? '#' + p.id : '')); p = p.parentElement; } const container = el.closest('div[class*="modal"], div[class*="dialog"], div[class*="drawer"], div[class*="overlay"]') || document.body; const btns = container.querySelectorAll('button'); for (const btn of btns) { if (btn.offsetParent === null || btn.disabled) continue; const bt = btn.textContent.trim(); if (['确认发布','确认提交','确认','确定','确定提交','提交','发布','我知道了','知道了'].some(s => bt.includes(s))) { btn.click(); return { action: 'clicked:' + bt, chain: ancestorChain, containerTag: container.tagName + (container.className && typeof container.className === 'string' ? '.' + container.className.split(' ').filter(c => c).join('.') : ''), matchedText: dt }; } } return { chain: ancestorChain, matchedText: dt, containerTag: container.tagName + (container.className && typeof container.className === 'string' ? '.' + container.className.split(' ').filter(c => c).join('.') : ''), noButton: true }; } } } return null; }); if (mhResult) { log('info', 'publish confirm modal handled', { action: mhResult.action || 'no_button', chain: mhResult.chain, containerTag: mhResult.containerTag, matchedText: mhResult.matchedText }); if (mhResult.action) { await sleep(4000); continue; } } } catch (e) {}
            try { const sc2 = await page.evaluate(() => { const allEls = document.querySelectorAll('button, a, span[class*="btn"], div[class*="btn"]'); for (const btn of allEls) { if (btn.offsetParent === null || btn.disabled) continue; const t = btn.textContent.trim(); if (t === '确认发布' || t === '提交' || t === '发布') { btn.click(); return t; } } return null; }); if (sc2) { log('info', 'publish submit clicked', { action: sc2, attempt: submitAttempt }); await sleep(4000); continue; } } catch (e) {}
            let clicked = await page.evaluate(() => { const allEls = document.querySelectorAll('button, a, span[class*="btn"], div[class*="btn"]'); const isInModal = (el) => { let p = el; while (p) { if (p.className && typeof p.className === 'string' && /modal|dialog|popup|drawer|overlay/.test(p.className)) return true; p = p.parentElement; } return false; }; for (const btn of allEls) { if (btn.offsetParent === null || btn.disabled) continue; const t = btn.textContent.trim(); if ((t === '确定' || t === '知道了') && isInModal(btn)) { btn.click(); return 'modal:' + t; } } for (const btn of allEls) { if (btn.offsetParent === null || btn.disabled) continue; const t = btn.textContent.trim(); if (t === '忽略全部') { btn.click(); return t; } } for (const btn of allEls) { if (btn.offsetParent === null || btn.disabled) continue; const t = btn.textContent.trim(); if (t === '下一步') { btn.click(); return '下一步'; } } for (const btn of allEls) { if (btn.offsetParent === null || btn.disabled) continue; const t = btn.textContent.trim(); if (t === '确定' || t === '知道了') { btn.click(); return t; } } return null; });
            if (!clicked) { try { const fnd = await page.evaluate(() => { const xpath = "//button[text()='提交']"; const el = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue; if (el) { el.click(); return 'xpath_提交'; } return null; }); if (fnd) clicked = fnd; } catch (e) {} }
            if (clicked) { log('info', 'publish step clicked', { action: clicked, attempt: submitAttempt }); } else { log('warn', 'no publish button found at step', { attempt: submitAttempt }); }
            await sleep(3000);
        }

        if (!publishSuccessConfirmed) { await takeScreenshot(page, 'fanqie_publish_fail'); throw new Error('发布失败：未检测到发布成功标识'); }

        let postId = itemId || capturedItemId;
        const isPublished = await page.evaluate(() => {
            return window.location.href.includes('/chapter-manage/');
        });
        if (!isPublished) {
            await takeScreenshot(page, 'fanqie_publish_fail'); throw new Error('发布失败：未跳转到章节管理页面');
        }
        if (!postId) {
            postId = await getChapterIdFromURL(page);
            if (!postId) { await takeScreenshot(page, 'fanqie_publish_fail'); throw new Error('发布失败：无法提取章节ID'); }
        }
    ...
    for (let attempt = 0; attempt <= CONFIG.MAX_RETRIES; attempt++) {
        ...
        try {
            ...
            switch (action) {
                ...
                case 'publish':
                default:
                    result = await publishWithTimeout(cookieStr, input);
                    break;
            }
...
async function doPublish(cookieStr, input) {
    ...
    try {
        ...
        log('info', 'filling chapter title', { title, chapterNumber: effectiveChapterNumber });
        ...
        log('info', 'filling chapter content', { contentLen: cleanContent.length });
        await fillContent(page, cleanContent);
        await sleep(1000);
...
/**
 * 注入 cookie 并导航到作家专区，确认已登录。
 * 组合了 parseCookieString + page.goto + checkLogin。
 */
...
async function takeScreenshot(page, name) {
    try {
        const path = '/tmp/logs/' + name + '_' + Date.now() + '.png';
        await page.screenshot({ path, fullPage: false });
        log('info', 'screenshot saved', { path });
    } catch (e) {
        log('warn', 'screenshot failed', { name, error: e.message });
    }
}
...
async function dumpPageState(page, label) {
    try {
        return await page.evaluate((lbl) => {
            ...
            const visibleBtns = Array.from(document.querySelectorAll('button, a[class*="btn"], span[class*="btn"]'))
                .filter(el => el.offsetParent !== null)
                .map(el => (el.textContent || '').trim())
                .filter(t => t.length > 0 && t.length < 15)
                .slice(0, 15);
            return { label: lbl, url: window.location.href, bodyPreview, visibleBtns };
        }, label);
    } catch (e) {
        return { label, url: '', bodyPreview: '', visibleBtns: [], error: e.message };
    }
}
 ...
 * 检查番茄平台是否触发了每日发布上限。
 */
async function checkDailyLimit(page) {
    try {
        return await page.evaluate(() => {
            const txt = document.body ? document.body.innerText : '';
            if (txt.includes('已达上限') || txt.includes('今日发布次数已达') || txt.includes('发布次数已用完') ||
                txt.includes('已达每日上限') || txt.includes('超过每日上限') || txt.includes('每日发布数量') || txt.includes('超过上限') ||
                txt.includes('提交次数') || (txt.includes('今日发布') && txt.includes('上限'))) {
                return true;
            }
            return false;
        });
...
async function enterNovelByWorkId(page, workId, novelName) {
    log('info', 'entering novel by workId', { workId });
    await page.goto('https://fanqienovel.com/main/writer/', { waitUntil: 'domcontentloaded', timeout: CONFIG.NAVIGATION_TIMEOUT });
    await sleep(3000);
    await dismissPopups(page);
    await clickButton(page, ['作品管理']);
    await sleep(2000);
    const foundOnPage = await findNovelByName(page, novelName);
    if (foundOnPage) {
        await clickChapterManagement(page, novelName);
        return true;
    }
...
async function clickDraftByTitle(page, draftTitle) {
    const normalize = (s) => s.replace(/[\s\u200B\u00A0]+/g, ' ').trim();
    const chMatch = draftTitle.match(/第\s*([\d一二三四五六七八九十百千]+)\s*章/);
    const chNum = chMatch ? chMatch[1] : '';

    const markEditBtn = "function(r) { const row = r.closest('tr') || r.closest('div[class*=\"row\"]') || r.closest('div[class*=\"item\"]') || r.closest('li') || r.parentElement; if (!row) return ''; const btns = row.querySelectorAll('a, button, span, div'); for (const b of btns) { const href = b.getAttribute('href') || ''; if (href.includes('/publish/') && b.offsetParent) { return 'href|' + href; } } const iconBtns = row.querySelectorAll('span[class*=\"edit\"], span[class*=\"draft-edit\"], span[class*=\"icon-edit\"], i[class*=\"edit\"]'); for (const b of iconBtns) { if (b.offsetParent) { const href = b.parentElement.getAttribute('href') || ''; if (href.includes('/publish/')) { return 'href|' + href; } b.parentElement.setAttribute('data-publish-edit', 'true'); return 'icon_parent'; } } for (const b of btns) { if (!b.offsetParent || b.tagName === 'TH') continue; const t = (b.textContent || '').trim(); if (t === '\u7f16\u8f91' || t === '\u7ee7\u7eed\u7f16\u8f91' || t === '\u5199' || t === '\u4fee\u6539') { const href = b.getAttribute('href') || ''; if (href.includes('/publish/')) { return 'href|' + href; } b.setAttribute('data-publish-edit', 'true'); return 'text_exact'; } } for (const b of btns) { if (!b.offsetParent) continue; const inner = b.innerHTML || ''; if (inner.includes('\u270f') || inner.includes('\u270e') || inner.includes('\ud83d\udcdd') || inner.includes('pencil')) { b.parentElement.setAttribute('data-publish-edit', 'true'); return 'emoji'; } } return ''; }";

    const levels = [
        { name: 'normalized_full', desc: '\u5f52\u4e00\u5316\u5168\u6587\u5339\u914d',
          fn: async () => page.evaluate((target, normFnText, markFn) => {
            const norm = eval('(' + normFnText + ')'); const mark = eval('(' + markFn + ')'); const t = norm(target);
            const els = document.querySelectorAll('tr, li, div[class*="row"], div[class*="item"], div[class*="chapter"], a[href*="/writer/"]');
            for (const el of els) { if (!el.offsetParent) continue; const text = norm(el.textContent || ''); if (text.includes(t)) { const tag = mark(el); return { method: 'normalized', found: text.substring(0, 80), marked: tag }; } }
            return null;
          }, draftTitle, normalize.toString(), markEditBtn)
        },
        { name: 'chapter_number', desc: '\u7ae0\u53f7\u6570\u5b57\u5750\u6807',
          fn: async () => {
            if (!chNum || !/^\d+$/.test(chNum)) return null;
            return page.evaluate((num, markFn) => {
              const mark = eval('(' + markFn + ')'); const rows = document.querySelectorAll('tr, li, div[class*="row"], div[class*="item"], div[class*="chapter"]');
              for (const row of rows) { if (!row.offsetParent) continue; const text = (row.textContent || '').trim(); const m = text.match(/第\s*(\d+)\s*章/); if (m && m[1] === num) { const tag = mark(row); return { method: 'ch_num:' + num, found: text.substring(0, 80), marked: tag }; } }
              return null;
            }, chNum, markEditBtn)
          }
        }
        ...
        for (const level of levels) {
            try {
                ...
                if (result && result.marked) {
                    log('info', 'draft matched: ' + level.desc, { draftTitle, attempt, ...result });
                    if (result.marked.startsWith('href|')) {
                        const href = result.marked.substring(5);
                        await page.goto(CONFIG.BASE_URL + href, { waitUntil: 'domcontentloaded', timeout: CONFIG.NAVIGATION_TIMEOUT });
                    } else {
                        try { await page.click('[data-publish-edit="true"]'); } catch (ce) {}
                    }
                    return true;
                }
            } catch (e) { log('warn', 'draft match level failed', { level: level.name, error: e.message }); }
        }
        await sleep(2000);
    }
    return false;
...
function extractItemIdFromURL(url) {
    const m = url && url.match(/\/publish\/(\d+)/);
    return m ? m[1] : '';
}
...
async function handlePublishSettings(page) {
    ...
    // 1.1) 处理分卷选择后可能弹出的确认弹窗
    try {
        await page.evaluate(() => {
            const modals = document.querySelectorAll('[class*="modal"], [class*="dialog"], [class*="popup"], [class*="drawer"], [class*="overlay"], [role="dialog"]');
            for (const modal of modals) {
                if (modal.offsetParent === null) continue;
                const btns = modal.querySelectorAll('button');
                for (const btn of btns) {
                    if (btn.offsetParent === null || btn.disabled) continue;
                    const t = (btn.textContent || '').trim();
                    if (t === '确认' || t === '确定' || t === '知道了') { btn.click(); return true; }
                }
            }
            return false;
        });
