...
/** 不使用 Radix Overlay（其内置 scroll-lock 会补 padding-right 导致页面横向抖动） */
  ...
  return (
    <DialogPortal>
      ...
      >
        ...
        >
          {children}
          <DialogPrimitive.Close className="absolute right-4 top-4 rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-orange-500/40 focus:ring-offset-2 disabled:pointer-events-none">
            <X className="h-4 w-4" />
...
const DialogHeader = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn("flex flex-col space-y-1.5 text-center pb-2", className)} {...props} />
)
